    ...
    for (int i3 = start3; i3 < stop3; ++i3) {
        for (int i2 = start2; i2 < stop2; ++i2) {
            for (int i1 = start1; i1 <= stop1; ++i1) {
                ...
                std::array<double, 5> UL = {tileQ[cL + var::rho * tileSize],
                                            tileQ[cL + var::rhou * tileSize],
                                            tileQ[cL + var::rhov * tileSize],
                                            tileQ[cL + var::rhow * tileSize],
                                            tileQ[cL + var::E * tileSize]};

                std::array<double, 5> UR = {tileQ[cR + var::rho * tileSize],
                                            tileQ[cR + var::rhou * tileSize],
                                            tileQ[cR + var::rhov * tileSize],
                                            tileQ[cR + var::rhow * tileSize],
                                            tileQ[cR + var::E * tileSize]};

                auto flux = computeFaceFlux(UL, UR, fluxFunc);
                for (auto v :
                     {var::rho, var::rhou, var::rhov, var::rhow, var::E}) {
